...
export default function DollarToRbx() {
  ...
  // So 1 Dollar = 1000 / 12.50 = 80 RBX
  const calculateRBX = (dollarAmount: number): number => {
    const rbxPerDollar = 1000 / 12.50; // 80 RBX per dollar
    return dollarAmount * rbxPerDollar;
  };
  ...
  const handleCount = () => {
    ...
    const dollarAmount = parseFloat(dollar);
    ...
    const calculatedRBX = calculateRBX(dollarAmount);
          ...
          {showResult && result !== null && (
            <div className="mb-6 p-6 bg-[#1a1a1f] rounded-lg border border-orange-500">
              <div className="text-center">
                ...
                <p className="text-orange-500 text-3xl font-bold">{Math.round(result)} RBX</p>
                <p className="text-gray-400 text-sm mt-2">For ${dollar}</p>
              </div>
            </div>
          )}
