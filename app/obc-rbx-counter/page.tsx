...
export default function ObcRbxCounter() {
  const router = useRouter();
  const [days, setDays] = useState<string>('');
  const [result, setResult] = useState<number | null>(null);
  const [showResult, setShowResult] = useState(false);

  // OBC to RBX conversion logic: 1 day = 60 RBX
  const calculateRBX = (numberOfDays: number): number => {
    // Each day gives 60 RBX
    ...
    <div className="min-h-screen p-4">
      <div className="w-full max-w-md mx-auto">
        ...
        <div className="flex items-center gap-4 mb-6 mt-4">
          ...
          <h1 className="text-2xl font-bold text-white">OBC RBX Counter</h1>
        </div>
